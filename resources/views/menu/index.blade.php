...
<body class="bg-slate-50 font-sans antialiased text-slate-800">
    ...
    <div class="fixed top-4 right-4 z-[60] flex flex-col gap-2">
        @if(session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="bg-emerald-500 text-white px-6 py-3 rounded-xl shadow-lg font-bold flex items-center gap-2">
        ...
        @if(session('error'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)" x-transition class="bg-red-500 text-white px-6 py-3 rounded-xl shadow-lg font-bold flex items-center gap-2">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('cart.index') }}" class="relative flex items-center gap-2 {{ $cartItems > 0 ? 'bg-orange-50 hover:bg-orange-100 border border-orange-200 text-orange-600' : 'bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-800' }} px-5 py-2 rounded-full font-bold transition-colors">
            ...
            @foreach ($products as $product)
                <div class="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden flex flex-col border border-slate-100 group">
                    ...
                    <div class="p-5 flex flex-col flex-grow">
                        ...
                        <form action="{{ route('cart.add', $product->id) }}" method="POST" class="mt-5 w-full m-0 p-0">
                            @csrf
                            <button type="submit" class="w-full flex items-center justify-center gap-2 {{ $product->stock_quantity == 0 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-orange-500 hover:bg-orange-600 text-white shadow-md hover:shadow-lg hover:-translate-y-0.5' }} font-bold py-2.5 px-4 rounded-xl transition-all duration-200" {{ $product->stock_quantity == 0 ? 'disabled' : '' }}>
                                @if($product->stock_quantity > 0)
